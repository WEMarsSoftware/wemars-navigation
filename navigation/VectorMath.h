...
float projectVector(float vector[], float plane[], index){
    returnArray[a] = vector[index] - (dotProduct(vector,plane)/pow(magnitude(plane),2))*plane[index]; //calculate magnitude of projected vector component a 
}
